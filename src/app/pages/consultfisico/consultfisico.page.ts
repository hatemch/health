...
export class ConsultfisicoPage implements OnInit {
  ...
  public subtitle = 'Consultar exames físicos';
  ...
  MostraDados() {
    ...
    this.Authorizer.QueryStoreProc('ExecutarPost', 'spCRUDPaciente', params).then(res => {
      let resultado: any = res[0];
      console.log("resultado",resultado)
      try {
        if (resultado.success) { 
          this.a=  JSON.parse(atob(resultado.results));
        ...
        this.alertService.presentAlert({ pTitle: this.env.AppName, pSubtitle: this.subtitle, pMessage: 'Não exame' });
      }
    });
