...
//MODELS
import { MarcadoresFoto } from 'src/app/models/MarcadoresFoto';
//SERVICES
import { AuthService } from 'src/app/services/auth.service';
import { EnvService } from 'src/app/services/env.service';
import { AlertService } from 'src/app/services/alert.service';

import { FotoPage } from 'src/app/pages/foto/foto.page';

@Component({
  selector: 'app-marcadores-foto',
  templateUrl: './marcadores-foto.page.html',
  styleUrls: ['./marcadores-foto.page.scss'],
})
export class MarcadoresFotoPage implements OnInit {
  @ViewChild('inputcamera') cameraInput: ElementRef;
  @ViewChild('inputGallery') galleryInput: ElementRef;
  @ViewChild('imgresult') imgResult: ElementRef;
  public subtitle = 'Cadastrar Marcadores Foto ';

  public model: MarcadoresFoto = new MarcadoresFoto();
  public collectionFilter: MarcadoresFoto[];

  public fotos: Array<MarcadoresFoto> = [];
  ...
  ngOnInit() {
    this.activatedRoute.params.subscribe((res) => {
      ...
      this.model.tipo = '1';
      this.model.marcadores = res.codigoMarcador;
      this.CRUDMarcadoresFoto('spCRUDMarcadoresFoto', 'READ', { marcadores: this.model.marcadores, tipo: this.model.tipo });
    });
  ...
  CRUDMarcadoresFoto(storeProcedure: string, statusCRUD: string, formValues: any) {
  ...
  salvarImagem(formData: FormData) {
    this.Authorizer.QueryStoreImagem('SalvarImagem', '', this.env.IMAGE_PRODUCT, formData).then(res => {
      ...
      if (resultado.success) {
        ...
        this.CRUDMarcadoresFoto('spCRUDMarcadoresFoto', 'CREATE', this.model);
      } else {
  ...
  // functions for the web version of the app functionalitie of store images 
  ...
  salvarImagemWebVersion(formData: FormData) {
    if (this.fotos.length == 3) {
      this.alertService.presentAlert({ pTitle: this.env.AppNameSigla, pSubtitle: this.subtitle, pMessage: 'Número máximo de fotos atingido' });
      this.foto = [];
      return;
    }
